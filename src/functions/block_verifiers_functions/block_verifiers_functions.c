...
int start_blocks_create_data(char* message, char* network_block_string)
{
  ...
  production_settings == 0 ? memcpy(blockchain_data.blockchain_reserve_bytes.block_producer_public_address,NETWORK_DATA_NODE_1_PUBLIC_ADDRESS,XCASH_WALLET_LENGTH) : memcpy(blockchain_data.blockchain_reserve_bytes.block_producer_public_address,NETWORK_DATA_NODE_1_PUBLIC_ADDRESS_PRODUCTION,XCASH_WALLET_LENGTH);
...
/*
-----------------------------------------------------------------------------------------------------------
Name: data_network_node_create_block_data
Description: The main network data node will create the blocks data
Parameters:
  message - The blocks data
Return: 0 if an error has occured, 1 if successfull
-----------------------------------------------------------------------------------------------------------
*/

int data_network_node_create_block_data(char *message)
{
  // Variables
  char data[BUFFER_SIZE];
  char data2[BUFFER_SIZE];
  ...
  // define macros
  #define DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR(settings) \
  memcpy(error_message.function[error_message.total],"data_network_node_create_block_data",35); \
  memcpy(error_message.data[error_message.total],settings,sizeof(settings)-1); \
  error_message.total++; \
  return 0;
  ...
  if (get_block_template(data) == 0)
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not get a block template");
  }

  // convert the network_block_string to blockchain_data
  if (network_block_string_to_blockchain_data((const char*)data,"0",BLOCK_VERIFIERS_AMOUNT) == 0)
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not convert the network_block_string to blockchain_data");
  }
  ...
  if (create_random_VRF_keys(VRF_data.vrf_public_key,VRF_data.vrf_secret_key) == 1 && crypto_vrf_is_valid_key((const unsigned char*)VRF_data.vrf_public_key) != 1)
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not create the vrf_public_key or vrf_secret_key");
  }
  ...
  if (crypto_vrf_prove(VRF_data.vrf_proof,(const unsigned char*)VRF_data.vrf_secret_key,(const unsigned char*)VRF_data.vrf_alpha_string_data,(unsigned long long)strlen((const char*)VRF_data.vrf_alpha_string_data)) != 0)
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not create the vrf proof");
  }
  if (crypto_vrf_proof_to_hash(VRF_data.vrf_beta_string,(const unsigned char*)VRF_data.vrf_proof) != 0)
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not create the vrf beta string");
  }
  if (crypto_vrf_verify(VRF_data.vrf_beta_string,(const unsigned char*)VRF_data.vrf_public_key,(const unsigned char*)VRF_data.vrf_proof,(const unsigned char*)VRF_data.vrf_alpha_string_data,(unsigned long long)strlen((const char*)VRF_data.vrf_alpha_string_data)) != 0)
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not create the VRF data");
  }
  ...
  if (blockchain_data_to_network_block_string(VRF_data.block_blob,BLOCK_VERIFIERS_AMOUNT) == 0)
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not convert the blockchain_data to a network_block_string");
  }
  ...
  {
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not sign the network block string");
  }
  ...
  memcpy(message+103,VRF_data.block_blob,strnlen(VRF_data.block_blob,BUFFER_SIZE));
  memcpy(message+strlen(message),"\",\r\n}",5);
  
  // sign_data
  if (sign_data(message) == 0)
  { 
    DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR("Could not sign_data");
  }
  return 1;

  #undef DATA_NETWORK_NODE_CREATE_BLOCK_DATA_ERROR
}
...
/*
-----------------------------------------------------------------------------------------------------------
Name: data_network_node_verify_block_data
Description: The main network data node will verify the block data and add the block verifiers signatures to the block
Parameters:
  message - The database data to send to the block verifiers
  network_block_string - The network block to submit to the network
Return: 0 if an error has occured, 1 if successfull
-----------------------------------------------------------------------------------------------------------
*/

int data_network_node_verify_block_data(char *message, char *network_block_string)
{
  // Variables
  char data[BUFFER_SIZE];
  char data2[BUFFER_SIZE];
  ...
  // define macros
  #define DATA_NETWORK_NODE_VERIFY_BLOCK_DATA_ERROR(settings) \
  memcpy(error_message.function[error_message.total],"data_network_node_verify_block_data",35); \
  memcpy(error_message.data[error_message.total],settings,sizeof(settings)-1); \
  error_message.total++; \
  return 0;
  ...
  if (blockchain_data_to_network_block_string(VRF_data.block_blob,BLOCK_VERIFIERS_AMOUNT) == 0)
  {
    DATA_NETWORK_NODE_VERIFY_BLOCK_DATA_ERROR("Could not convert the blockchain_data to a network_block_string");
  }

  // add the data hash to the network block string
  if (add_data_hash_to_network_block_string(VRF_data.block_blob,network_block_string) == 0)
  {
    DATA_NETWORK_NODE_VERIFY_BLOCK_DATA_ERROR("Could not add the network block string data hash");
  }
  ...
  if (sign_data(message) == 0)
  { 
    DATA_NETWORK_NODE_VERIFY_BLOCK_DATA_ERROR("Could not sign_data");
  }
  return 1;

  #undef DATA_NETWORK_NODE_VERIFY_BLOCK_DATA_ERROR
}
...
int data_network_node_create_block(void)
{
  ...
  {   
    ...
    // create the block
    if (data_network_node_create_block_data(data) == 0)
    {
      DATA_NETWORK_NODE_CREATE_BLOCK_ERROR("Could not create the network block");
    }
    ...
    // wait for the block verifiers to receive the block and create the block verifiers signature
    ...
    color_print("Part 2 - Verify the block and send the block verifiers the database data","yellow");

    // create the block with the block verifiers signatures
    if (data_network_node_verify_block_data(data,data2) == 0)
    {
      DATA_NETWORK_NODE_CREATE_BLOCK_ERROR("Could not create the network block with the block verifiers signatures");
    }
    ...
    if (submit_block_template(data2) == 0)
    {
      ...
      memset(data2,0,sizeof(data2));
      memcpy(data,"{\"block_height\":\"",17);
      memcpy(data+17,current_block_height,strnlen(current_block_height,sizeof(data)));
      memcpy(data+strlen(data),"\"}",2);
...
int block_verifiers_create_block(void)
{
  ...
  start:
    ...
    // wait for the block verifiers to process the votes
    sync_block_verifiers_seconds(current_date_and_time,current_UTC_date_and_time,START_TIME_SECONDS_NETWORK_BLOCK_PART_2);
    ...
    // wait for the block verifiers to process the votes
    sync_block_verifiers_seconds(current_date_and_time,current_UTC_date_and_time,START_TIME_SECONDS_NETWORK_BLOCK_PART_3);
    ...
    // wait for the block verifiers to process the votes
    sync_block_verifiers_seconds(current_date_and_time,current_UTC_date_and_time,START_TIME_SECONDS_NETWORK_BLOCK_PART_4);
    ...
    // wait for the block verifiers to process the votes
    sync_block_verifiers_seconds(current_date_and_time,current_UTC_date_and_time,SEND_DATA_TIME_SECONDS_NETWORK_BLOCK_PART_4);
